...
for id in ra_dict:
    ...
    if search_included_snowballing==0:
        print(id)
    seed_p.append(seed/len(ra_files))
    search_p.append(search/len(ra_files))
    seed_snowballing_p.append(seed_snowballing/len(ra_files))
    seed_snowballing_search_p.append(seed_snowballing_search/len(ra_files))
    search_included_snowballing_p.append(search_included_snowballing/len(ra_files))
    seed_snowballing_search_snowballing_p.append(seed_snowballing_search_snowballing/len(ra_files))

print(statistics.mean(seed_p))
...
with plt.rc_context({
    ...
    plt.yticks([1, 2, 3, 4, 5, 6], ['Seed', 'Seed-snowballed','Retrieved-set','(Seed-snowballed)+(Retrieved-set)', '(Retrieved-set)+(Screened-snowballed)', '(Seed-snowballed)+(Retrieved-set)+(Screened-snowballed)'], fontsize=18)
    plt.xlabel("Recall", fontsize=20)
    plt.xlim([-0.1, 1.1])
    plt.ylabel("Retrieval method", fontsize=20)
    plt.tight_layout()
    plt.savefig("graph/box_plot_recall.pdf")
    print(statistics.mean([len(x) for x in sfs_dict.values()]))
    print(statistics.mean([len([v for v in sfi_dict[key] if v not in searching_dict[key]]) for key in sfi_dict]))